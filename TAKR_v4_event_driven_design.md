...
Phase 1: 基因级事件 — 解决非线性结构
  - indel/loss — 孩子A 有 A-B-C，孩子B 有 A-C
    外类群投票:
    · 外类群有 A-B-C → 祖先 = A-B-C → B丢失是 derived → 移除跨越边 A-C（孩子B的边）
    · 外类群有 A-C   → 祖先 = A-C   → B插入是 derived → 移除路径边 A-B, B-C（孩子A的边）
    不移除 HOG，只移除衍生边
  - duplication（重复）— 合并拷贝、简化并行边
  - 处理后图应是线性路径（无分支、无环）

Phase 2: 小规模结构重排（染色体数不变）
  - telomere_inversion（端粒倒位）— 优先处理！改变端粒位置
  - internal_inversion（内部倒位）
  - unidir_trans（单向易位）

Phase 3: 大尺度重排（染色体数改变）
  - EEJ（端粒-端粒连接）→ 染色体数-1
  - NCF（嵌套染色体融合）→ 染色体数-1
  - fission（断裂）→ 染色体数+1
  - RT（相互易位）→ 染色体数不变
...
| 事件 | 祖先图 | 子代图 | 断边 | 新边 | 图特征 |
|------|--------|--------|------|------|--------|
| internal_inversion | T1-A-B-C-D-E-F-T2 | T1-A-E-D-C-B-F-T2 | A-B, E-F | A-E, B-F | 内部block倒转 |
| telomere_inversion | T1-A-B-C-D-E-T2 | T1-E-D-C-B-A-T2 | T1-A, D-E | T1-E, D-A | block含端粒 |
| unidir_trans | T1-A-B-C-D-T2, T3-E-F-G-H-T4 | T1-A-T2, T3-E-B-C-D-F-G-H-T4 | A-B, D-T2, E-F | A-T2, E-B, D-F | 单向移动，供体丢失 |

### 3.3 大尺度重排（染色体数改变）

| 事件 | 祖先图 | 子代图 | 断边 | 新边 | 染色体数 | 图特征 |
|------|--------|--------|------|------|---------|--------|
| EEJ | T1-A-B-T2, T3-C-D-T4 | T1-A-B-C-D-T4 | — | B-C | -1 | 不同染色体的端粒邻接 |
| NCF | T1-A-B-C-D-T2, T3-E-F-G-T4 | T1-A-B-E-F-G-C-D-T2 | B-C | B-E, G-C | -1 | 一条整链插入另一条内部 |
| fission | T1-A-B-C-D-T2 | T1-A-B-T3, T4-C-D-T2 | B-C | B-T3, T4-C | +1 | 一个邻接断开，两端成端粒 |
| RT | T1-A-B-C-D-T2, T3-E-F-G-H-T4 | T1-A-B-G-H-T4, T3-E-F-C-D-T2 | B-C, F-G | B-G, F-C | 0 | 交叉交换 |
| URT | T1-A-B-C-D-T2, T3-E-F-G-H-T4 | T1-A-B-G-H-T4, T3-E-F-C-D-T2 | B-T2, F-G | B-G, F-T2 | 0 | 同RT但一端在端粒 |
...
#### 第1步：单物种内部处理 — duplication resolve

duplication 是单个物种内部的图问题，**在跨物种比较之前**对每个 mapped child 单独处理。

对每个 mapped child 的邻接图：
```
1. 遍历所有节点，找度 > 2 的节点（可能分叉点）
2. 对每个分叉点，检查是否来自同一个物种的重复：
   - tandem dup: 相邻拷贝（B-B'），产生并行边 B→B' 和 B→C, B'→C
   - 合并: 只保留一个拷贝的路径，移除并行边
3. 记录 tandem_dup / dispersed_dup / proximal_dup 事件
4. 输出: 无并行边的线性图
```

```
处理前: A-B, B-B', B'-C (tandem dup B→B')
处理后: A-B, B-C (保留一条路径)
事件: tandem_dup on child1: B
```

resolve 后的图再去参与跨物种的共识构建。

#### 第2步：跨物种共识 + indel/loss

```
Step 2-1: 建初始共识图（无向图 + 颜色标签）
  duplicate-resolve 后的每个孩子贡献邻接集合
  共识图 G = 所有边的并集
  每条边记录颜色 (child_id, chromosome_id)

Step 2-2: indel/loss 检测
  遍历图，找跨越边（两个端点在另一个孩子中通过中间节点连接）
  跨越边 = 潜在 indel
  移除跨越边，记录 indel/gain/loss
...
```
剩余图中找环（DFS back-edge detection）
对每个环：
  1) 检查环两端颜色
  2) 检查端粒状态
  3) 判事件类型：
     - 颜色交替 + 同一染色体 → inversion
     - 颜色交替 + 跨染色体 → RT/URT
     - 非交替 + 跨越边连接两端粒 → EEJ
     - 非交替 + 插入段为完整染色体 → NCF
  4) 从图中移除冲突边
...
class ColoredGraph:
    ...
    def resolve_all_events(self, outgroups: Dict = None) -> List[TAKREvent]:
        ...
        1. duplication resolve (每个child单独)
        2. indel/loss resolve (find_spanning_edges → 外类群投票 → 移除)
        3. 结构重排 (循环: find_cycles → classify_cycle → 移除冲突边)
        4. path_cover → 验证
        """
        ...
```
