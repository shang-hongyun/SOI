...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-04-26 | v1.1 | 首次实现 | 初始版本 — 迭代计划 + AK.py 接口设计 |
| 2026-04-26 | v1.2 | **Outgroup voting for all event types**: `resolve_structural_events()` and `resolve_bridge_events()` now use outgroup adjacency at parent HOG level for polarity. Bridge: outgroup-has-adjacency→fission, else→EEJ/NCF, no-outgroup→bridge_unclassified. Structural: per-edge outgroup count replaces min-count heuristic. `_remove_rare_color_edge()` extracted. `takr_event_driven.py`: `outgroup_leaves_map` precomputation, maps sibling leaves to parent HOG level. |
| 2026-04-26 | v1.3 | **Synteny block compression**: `_build_synteny_blocks()` builds blocks from maximal shared-edge chains (687 HOGs→~30 blocks). `_compress_to_block_level()` creates block graph. `_classify_block_cycle()` handles block-level 4-cycle alternating patterns + 3-cycle inversion (2 unique edges from different children + 1 shared). Adjacent inversion 3-cycles merged. Pipeline: blocks → block-structural → block-bridge → HOG-structural → HOG-bridge → indels. |
| 2026-05-24 | v1.4 | **v4.12 fixes**: (1) RT detection: alternating check uses child-level comparison, not full color equality. (2) Outgroup adjacency: requires ALL outgroup species (per-leaf counting). (3) Bridge detection: telomere requirement restored after false fission regression. (4) Block compression: `child_telomere_set()` for all telomeres. (5) Per-phase event type logging via `Counter`. (6) `all_hogs()` O(n²)→O(n) caching. (7) Local import shadowing fix. (8) Indel filter debugging counters. (9) Child viz reuses `draw_block_graph`. (10) Node size log2 scaling. (11) Sister dotplots in simulator. (12) Graphviz layout params (nodesep/ranksep/splines). |
