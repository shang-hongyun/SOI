...
Phase 1: 每个孩子内部 — duplication resolve（所有长度）
  - tandem_dup, dispersed_dup, proximal_dup, seg_duplication
  - 每个孩子单独简化，去掉重复边
  - 然后两个孩子合图（彩色图）
  - **Postcondition**: 每个孩子内部图是线性染色体集合
    · 每个节点 degree ≤ 2（路径或环）
    · 无并行边（同一对节点之间只有一条边）
    · 无分支节点（degree > 2）
    · 验证: `assert all(deg <= 2 for deg in node_degrees)`

Phase 2: 单基因 indel/loss/gain（HOG 级）— 最先跨物种处理
  ...
  - gene_gain — 标记孤立 HOG
  - 处理后图应是线性路径（无分支、无环）
  - **Postcondition**: 彩色图中无跨越边
    · 任意边的两个端点，在另一个孩子中不存在通过中间节点的路径（无隐藏的 indel）
    · 无 degree > 2 的节点（无分支）
    · 无环（2-cycles 已全部解决）
    · 验证: `assert no_cross_edges(graph)` and `assert no_cycles(graph)`

Phase 3: 共线性块压缩
  - 从简化的 HOG 级图构建块
  - 块 = 共享边的连续路径
  - 块级图用于后续大规模重排检测
  - **Postcondition**: 块级图构建完成
    · 每个 HOG 恰好属于一个块
    · 块内节点 degree ≤ 2（线性路径）
    · 块间边正确记录颜色
    · 验证: `assert all_hogs_assigned_to_blocks(graph)`

Phase 4: 块级事件（按复杂度递增，确保图足够简化后再处理复杂事件）

  4a. seg_deletion / seg_insertion — 最简单的块级事件
    - 一个孩子有大段，另一个没有
    - 外类群投票区分 deletion vs insertion
    - **Postcondition**: 无大段不对称（一个孩子有连续 N 个块，另一个完全缺失）
      · 验证: `assert no_large_asymmetric_segments(graph, min_len=3)`

  4b. unidir_trans — 单向转移
    - 一条染色体的片段转移到另一条
    - 供体染色体丢失
    - **Postcondition**: 无单向转移模式
      · 不存在: A 有 a-b-c-d，B 有 a-x-d（b,c 被 x 替换，且 x 只在 B 中）
      · 验证: `assert no_unidirectional_transfers(graph)`

  4c. telomere_inversion — 端粒倒位（优先处理！）
    - 一个断点在端粒，改变端粒位置
    - 必须先于其他 inversion 处理，否则影响后续端粒检测
    - **Postcondition**: 所有端粒位置已确定，不再变化
      · 不存在: 一个孩子端粒在 X，另一个孩子端粒在 Y（同一祖先端粒）
      · 验证: `assert telomere_positions_consistent(graph)`

  4d. internal_inversion — 内部倒位
    - 两个断点都在染色体内部
    - 不改变端粒位置
    - **Postcondition**: 块级图中无环（所有 2-cycles 已解决）
      · 验证: `assert no_cycles(block_graph)`

  4e. RT / URT — 相互易位（最复杂的结构重排）
    - 两条染色体交叉交换片段
    - 用 chrom_idx 区分 RT 和 inversion
    - 用端粒参与区分 RT 和 URT
    - **Postcondition**: 块级图中无跨染色体的环
      · 不存在: 两个端点 chrom_idx 不同的 2-cycle
      · 验证: `assert no_cross_chrom_cycles(block_graph)`

  4f. EEJ / NCF / fission — 桥接事件（改变染色体数，最后处理）
    - EEJ: 端粒-端粒连接 → 染色体数-1
    - NCF: 嵌套染色体融合 → 染色体数-1
    - fission: 断裂 → 染色体数+1
    - 外类群投票确定极性
    - 桥接检测在所有结构重排之后，避免 inversion/RT 断点被误判为桥
    - **Postcondition**: 无桥接边
      · 所有唯一边的两端在同一连通分量内
      · 验证: `assert no_bridge_edges(graph)`

Phase 5: 端粒约束路径覆盖
  - 共识端粒作为染色体锚点
  - 每条染色体恰好 2 个端粒
  - **Postcondition**: 所有 HOG 被覆盖，每条染色体恰好 2 个端粒
    · 无孤立节点（每个 HOG 至少在一个染色体中）
    · 无多余端粒（每条染色体恰好 2 个 degree-1 节点）
    · 验证: `assert all_hogs_covered(graph)` and `assert each_chrom_2_telomeres(graph)`
